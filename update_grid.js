...
// I will create a manual map of all products to avoid parsing issues
const allProductObjects = {
  ...
  "quanto-revit": {
    id: "revit-to-boq", // Need to match ID exactly or update
...
const grid = {
  architects: {
    "architecture-and-modelling": ["hand-drawn-to-autocad", "auto-conversion-2d-to-3d", "wordtobim"],
    "feasibility": ["planning-law-chatbot"],
    "tendering": ["builderbot"],
    "construction-stage": ["buildmonitor"],
    "claims-and-legal-disputes": ["builderbot"]
  },
  "real-estate-developers": {
    "architecture-and-modelling": ["wordtobim"],
    "feasibility": ["planning-law-chatbot", "cost-plan-calculator"],
    "boq-preparation": ["quanto-revit", "quanto-acc", "quanto-costx", "quanto-2d", "costx-to-boq", "auto-reinforcement", "buildmarketlk"],
    "tendering": ["builderbot"],
    "construction-stage": ["buildmonitor", "measureonair", "auto-reinforcement"],
    "claims-and-legal-disputes": ["builderbot"]
  },
  contractors: {
    "architecture-and-modelling": ["wordtobim"],
    "feasibility": ["planning-law-chatbot"],
    "boq-preparation": ["quanto-2d", "costx-to-boq", "auto-reinforcement", "buildmarketlk"],
    "tendering": ["builderbot", "tender-evaluations", "buildmarketlk"],
    "construction-stage": ["buildmonitor", "measureonair", "auto-reinforcement"]
  },
  "construction-consultancies": {
    "architecture-and-modelling": ["auto-conversion-2d-to-3d"],
    "feasibility": ["cost-plan-calculator", "prelim"],
    "boq-preparation": ["quanto-revit", "quanto-acc", "quanto-costx", "quanto-2d", "costx-to-boq", "auto-reinforcement", "buildmarketlk"],
    "tendering": ["tender-evaluations", "buildmarketlk", "prelim"],
    "construction-stage": ["measureonair", "auto-reinforcement"],
    "claims-and-legal-disputes": ["builderbot"]
  },
  modellers: {
    "architecture-and-modelling": ["hand-drawn-to-autocad", "auto-conversion-2d-to-3d", "wordtobim"],
    "feasibility": ["prelim"],
    "boq-preparation": ["auto-reinforcement"]
  },
  "legal-professionals": {
    "boq-preparation": ["buildmarketlk"],
    "construction-stage": ["buildmonitor"],
    "claims-and-legal-disputes": ["builderbot"]
  }
};
...
output += 'export interface HandoffConnector {\n  fromStage: string\n  toStage: string\n  text: string\n  pillText: string\n}\n\n';
output += 'export interface Stage {\n  id: string\n  label: string\n  headline: string\n  description: string\n  products: Product[]\n}\n\n';
output += 'export interface ComparisonColumn {\n  label: string\n  description: string\n  highlight?: boolean\n}\n\n';
output += 'export interface ComparisonSection {\n  title: string\n  columns: ComparisonColumn[]\n}\n\n';
output += 'export interface PersonaData {\n  id: string\n  url: string\n  eyebrow: string\n  heroTitle: string\n  heroTitleNormal?: string\n  heroTitleHighlight?: string\n  heroSubtext: string\n  stages: Stage[]\n  handoffs: HandoffConnector[]\n  comparison?: ComparisonSection\n  footerTitle: string\n  footerSubtext: string\n}\n\n';

output += 'export const personasData: Record<string, PersonaData> = {\n';

// The copy for personas to keep them intact
...
for (const pKey in grid) {
  output += `  "${pKey}": {\n`;
  output += `    id: "${pKey}",\n`;
  output += `    url: "${personaHeaders[pKey].url}",\n`;
  output += `    eyebrow: "${personaHeaders[pKey].eyebrow}",\n`;
  output += `    heroTitle: "${personaHeaders[pKey].heroTitle}",\n`;
  output += `    heroTitleNormal: "${personaHeaders[pKey].heroTitleNormal}",\n`;
  output += `    heroTitleHighlight: "${personaHeaders[pKey].heroTitleHighlight}",\n`;
  output += `    heroSubtext: "${personaHeaders[pKey].heroSubtext}",\n`;
  output += `    stages: [\n`;
  
  for (const sKey in grid[pKey]) {
    const stageObj = STAGES[sKey];
    output += `      {\n`;
    output += `        id: "${stageObj.id}",\n`;
    output += `        label: "${stageObj.label}",\n`;
    output += `        headline: "${stageObj.headline}",\n`;
    output += `        description: "${stageObj.description}",\n`;
    output += `        products: [\n`;
    
    grid[pKey][sKey].forEach(prodId => {
      const prod = allProductObjects[prodId];
      if (!prod) {
        console.error("Missing product: " + prodId);
      }
      output += `          {\n`;
      output += `            id: "${prod.id}",\n`;
      output += `            title: "${prod.title}",\n`;
      output += `            painPoint: ${JSON.stringify(prod.painPoint)},\n`;
      output += `            features: ${JSON.stringify(prod.features)},\n`;
      output += `            targetMarket: ${JSON.stringify(prod.targetMarket)},\n`;
      output += `            status: "${prod.status}",\n`;
      output += `            pricing: ${JSON.stringify(prod.pricing)},\n`;
      output += `            videoUrl: "${prod.videoUrl}"${prod.requestDemoOnly ? ',\n            requestDemoOnly: true' : ''}${prod.visitUrl ? ',\n            visitUrl: "'+prod.visitUrl+'"' : ''}\n`;
      output += `          },\n`;
    });
  ...
  output += `    ],\n`;
  output += `    handoffs: [],\n`;
  if (personaHeaders[pKey].comparison) {
    output += `    comparison: ${JSON.stringify(personaHeaders[pKey].comparison, null, 6)},\n`;
  }
  output += `    footerTitle: "${personaHeaders[pKey].footerTitle}",\n`;
  output += `    footerSubtext: "${personaHeaders[pKey].footerSubtext}"\n`;
  output += `  },\n`;
}
